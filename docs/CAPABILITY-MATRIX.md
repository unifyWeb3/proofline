...
| Captured equivalence validator behavior | Direct runner | `DIRECT-UNIT-PROVEN`: `direct_vm.run_validator()` exercises the captured comparator's failed-leader rejection path. This is not committee execution. |
...
| Validator disagreement | GenLayer protocol | `HOSTED_ONLY / NOT PROVEN BY DIRECT MODE`. |
...
| Adapter consumption | Local adapter | `HOSTED-VERIFIED`: hosted receipts require a lifecycle client to re-observe and match the producing transaction/readback; repeat delivery is idempotent and edited verdicts are rejected. Direct receipts require explicit test-only opt-in. |
| Transaction/status client | Stable SDK | `IMPLEMENTED, READ-ONLY`: snake_case `get_transaction`, `wait_for_transaction_receipt`, `read_contract`, and `ExecutionResult`. |
| Hosted transaction | Studio Next chain 61997 | `HOSTED-VERIFIED`: corrected semantic deployment, registration, submission, finality, readback, fee, provenance, and receipt evidence are recorded in `evidence/milestone2-hosted-correction.json`. |
